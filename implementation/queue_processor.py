    ...
    def __process_queue(logger, event, job_queue_dispatcher, pool_size) -> None:
        ...
        while True:
            ...
            if len(jobs_to_run) > 0:
                logger.log_debug(
                    f'{len(jobs_to_run)} jobs ready to run',
            ...
            for job in jobs_to_run:
                if jobs_count >= pool_size:
                    logger.log_error(
                        'Executing of jobs is postponed. Job pool is full',
                        'JobQueueProcessor.__process_queue'
                    )
                    continue
                jobs_count += 1
                ...
                if job.error_message not in (None, ''):
                    logger.log_error(
                        ...
                        f' finished with error. Error message:'
                        f' "{job.error_message}"',
                        'JobQueueProcessor.__process_queue')
                if job.done:
                    logger.log_info(
            ...
            if event.is_set():
                logger.log_debug(
                    'Received kill event. Processing will be stopped',
                    'JobQueueProcessor.__process_queue'
                )
                break
    ...
    def run(self) -> None:
        ...
        self.__thread = threading.Thread(
            ...
            args=(
                self.__logger,
                self.__event,
                self.__job_queue_dispatcher,
                self.__pool_size,
            )
        )
        self.__thread.start()
        self.__is_running = True
        self.__logger.log_info('Queue processor is started')

    def stop(self) -> None:
        """Метод останавливает Job"""
        if not self.__is_running:
            self.__logger.log_debug('Queue processor is not running',
                                    'QueueProcessor.stop')
            return
